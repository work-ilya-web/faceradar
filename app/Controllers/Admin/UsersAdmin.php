...
class UsersAdmin extends BaseController
{
    ...
    {
        $loginLib = new AuthLib();
        $UsersModel = new UsersAdminModel();
        ...
        if ($loginLib->isAdmin()) {
            ...
            $data = [
                ...
                'admin' => true,
                'items' => $UsersList->paginate($in_page, 'users_main'),
        ...
        return view('profile/admin/users-main/list',  $data);
     ...
     */
     ...
     public function item_view($event = 'add',  $id = false)
     {
        ...
        if ($loginLib->isAdmin()) {
            switch ($event){
                case 'add':
                    $data = [
                        ...
                        'action' => site_url('Admin/UsersAdmin/add_ajax'),
                        'back_url' => site_url('profile/users-main'),
                    ];
                    break;
                case 'edit':
                    if(is_numeric($id)){
                        ...
                        if($userInfo == null){
                            ...
                            $data = [
                                ...
                                'user' => $userInfo,
                                'admin' => true,
                                'action' => site_url('Admin/UsersAdmin/edit_ajax'),
                                'cities' => $CitiesModel->RegisterCities(),
            ...
            return view('profile/admin/users-main/item',  $data);
        } else {
            throw new \CodeIgniter\Exceptions\PageNotFoundException();
        }
     }
    ...
    /*public function item_view(){
        ...
        if ($loginLib->isAdmin()) {
            if(in_array('edit', $segments)){
                $userInfo = $UsersModel->GetUser($last);
                if($userInfo == null){
                    throw new \CodeIgniter\Exceptions\PageNotFoundException();
                } else {
                    $data = [
    ...
    public function add_ajax(){
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $rules = [
                ...
                'phone' => 'required|min_length[3]|max_length[20]|is_unique[users_main.phone]',
                'email' => 'required|min_length[3]|max_length[50]|is_unique[users_main.email]',
